...
function LanguageBaseCommand() {
  ...
  const fetchData = (pageNum = 0, isRefresh = false) => {
    ...
    request(
      "GET",
      `/command/fetch-by-lang/5/${pageNum}/${filterLang}`,
      "application/json",
      {},
      "application/json"
    ).then((response) => {
    ...
    if (itemToDelete) {
      request(
        ...
        if (response.status === 200) {
          ...
          request(
            "GET",
            `/command/fetch-by-lang/5/0/${filterLang}`,
            "application/json",
            {},
            "application/json"
          ).then((response) => {
      ...
      <Card className="formbody mx-auto" style={{ maxWidth: "900px" }}>
        ...
        <Card.Header className="formheader">
          <div className="header-container">
            <h2 className="text-center my-3">Commands - {filterLang}</h2>
            <button
          ...
          <div className="animate-content">
            {Content && Content.length > 0 ? (
              ...
              <div className="text-center py-5">
                <p className="text-muted">No commands found for this language.</p>
              </div>
            )}
          </div>
